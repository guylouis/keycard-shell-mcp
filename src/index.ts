...
import { addressFromPublicKey, buildUnsigned, attachSignature, rawForShell } from "./eth.js";
    ...
    attempt(
      ...
      (i) =>
        [
          ...
          `  RPC endpoint:  ${RPC_URL ?? "not configured (signing works, broadcasting does not)"}`,
        ].join("\n"),
    ),
);
